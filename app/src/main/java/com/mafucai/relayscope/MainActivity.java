...
public final class MainActivity extends Activity {
    ...
    private void configureWebView(WebView view) {
        ...
        view.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public boolean onJsConfirm(android.webkit.WebView v, String url, String message, android.webkit.JsResult result) { result.confirm(); return true; }
...
@JavascriptInterface public void testGroup(String group) {
        ...
        private void noteFailStreak(String siteName, String status) {
            ...
            android.content.SharedPreferences p = getSharedPreferences("relayscope_stats", MODE_PRIVATE);
            String key = "rate_" + siteName;
            try {
                org.json.JSONObject o = new org.json.JSONObject(p.getString(key, "{\"ok\":0,\"total\":0}"));
                o.put("total", o.optInt("total") + 1);
                if ("可用".equals(status)) o.put("ok", o.optInt("ok") + 1);
                p.edit().putString(key, o.toString()).apply();
            } catch (Exception ignored) { }
        }

        private int successRate(String siteName) {
            try {
                org.json.JSONObject o = new org.json.JSONObject(getSharedPreferences("relayscope_stats", MODE_PRIVATE).getString("rate_" + siteName, "{\"ok\":0,\"total\":0}"));
                int total = o.optInt("total");
                return total == 0 ? -1 : Math.round(100f * o.optInt("ok") / total);
        ...
        private void pushState() {
            ...
            org.json.JSONArray rateArray = new org.json.JSONArray();
            for (RelaySite s : siteStore.load()) {
                int rate = successRate(s.name);
                if (rate >= 0) { JSONObject ri = new JSONObject(); try { ri.put("site", s.name); ri.put("rate", rate); rateArray.put(ri); } catch (Exception ignored) { } }
            }
        ...
        @JavascriptInterface public void startInspection(String minutesText, String group) {
            try {
                float minutes = Float.parseFloat(minutesText);
                if (!(minutes > 0) || Float.isInfinite(minutes)) throw new NumberFormatException();
                getSharedPreferences("relayscope", MODE_PRIVATE).edit().putFloat("interval", minutes).putString("inspection_group", group == null ? "" : group).apply();
                Intent intent = new Intent(MainActivity.this, InspectionService.class).putExtra("interval_minutes", minutes).putExtra("inspection_group", group == null ? "" : group);
                if (Build.VERSION.SDK_INT >= 26) startForegroundService(intent); else startService(intent);
                toast("巡检已开启" + (group == null || group.isEmpty() ? "（全站）" : "（分组：" + group + "）"));
